...
class CAdvancedArbehavior extends CActiveRecordBehavior
{
  ...
  Written in Yii relation syntax, it would be like this
	...
	{
		$relations = array();
                echo Yii::trace(CVarDumper::dumpAsString( $this->owner->relations()), 'RELATIONS');
	...
	{
		$key = $relation['key'];
                $m2mTableColumns = Yii::app()->db->schema->getTable($relation['m2mTable'])->columns;
		...
		if(!is_array($this->owner->$key) && $this->owner->$key != array()) 		
			$this->owner->$key = array($this->owner->$key);
                echo Yii::trace(CVarDumper::dumpAsString($this->owner->$key), 'ownerKey');
                
		// An array of objects is given
                $sql = "insert into ".$relation['m2mTable']." (".$relation['m2mThisField']
                                .", ".$relation['m2mForeignField'];
                
                foreach((array)$this->owner->$key as $index => $foreignobject)
		{
                        if(array_key_exists($index, $m2mTableColumns)){
                                //$matchedColumns[] = $index;
                                $matchedColumns[] = $foreignobject;
                                $sql .= ", ".$index;
                        }
                }
                $sql .= ") values (";
                $sqlHeader = $sql;
                echo Yii::trace(CVarDumper::dumpAsString((array)$this->owner->$key), 'array KEY');
                foreach((array)$this->owner->$key as $index => $foreignobject)
		{
...
//                        }
                        if(is_numeric($foreignobject)){
                                $sql .= '"'.$this->owner->{$this->owner->tableSchema->primaryKey}.'"';
                                $sql .= ", '".$foreignobject."'";
                                echo Yii::trace(CVarDumper::dumpAsString(count($matchedColumns)), 'Matched Columns');
                                for($i = 0;$i < count($matchedColumns);$i++)
                                        $sql .= ", '".$matchedColumns[$i][$foreignobject - 1]."'";
                                $sql .= ")";
                                echo Yii::trace(CVarDumper::dumpAsString($sql), 'INSERTSQLFULL');
                                $this->execute($sql);
                                $sql = $sqlHeader;
                        }
                }
